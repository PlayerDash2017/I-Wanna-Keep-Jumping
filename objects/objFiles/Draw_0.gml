for (var i = 0; i < global.total_saves; i++) {
    ...
    draw_set_color(c_black);
	draw_set_halign(fa_center);
    draw_text(400, y + spacing * i, string("Data{0}", i + 1));
    ...
    draw_text(400, y + 46 + spacing * i, draw_diff);
    
	//Deaths and Time
	draw_set_font(fntMenu3);
    draw_set_halign(fa_left);
    draw_text(400 - 200, y + 78 + spacing * i, string("Deaths: {0}", data.deaths));
    draw_set_halign(fa_right);
	draw_text(400 + 200, y + 78 + spacing * i, string("Time: {0}", formatted_time(data.time)));
      
	//Clear
    if (data.clear) {
		draw_set_font(fntMenu4);
        draw_set_halign(fa_center);
        draw_text(400, y + 92 + spacing * i, "Clear!");
    }
        
    if (select[0] == i) {
        draw_sprite(sprCherry, 0, 52*2, y + 60 + spacing * i);
        draw_sprite(sprCherry, 0, room_width - 56*2, y + 60 + spacing * i);
    }
