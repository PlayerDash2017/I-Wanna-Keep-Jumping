			...
			if (on_vine != null) {
			    ...
			    if ((on_vine.image_xscale == 1 && is_pressed(global.controls.right)) || (on_vine.image_xscale == -1 && is_pressed(global.controls.left))) {
			        if (is_held(global.controls.jump)) {
						...
						if (on_vine.object_index == objDisVine){
							jump_left = 0;
						}
...
//Cape color system
lerpCapeColor = lerp(lerpCapeColor,jump_left,0.1);
drawCapeColor = merge_color(capeColor[0],merge_color(capeColor[1],capeColor[2],max(lerpCapeColor-1,0)),min(lerpCapeColor,1))
#endregion
#endregion