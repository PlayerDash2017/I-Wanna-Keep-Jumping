function player_jump() {
	if (global.forms.vkid == 0) {
		...
		if (jump_total > 0 && (on_block != null || (platform != null && platform.visible) || on_platform || instance_place_check(x, y + global.grav, objWater1, tangible_collision) != null || on_ladder)) {
			...
			/*if (place_meeting(x,y+global.grav,objBlockDisfresher))
				{ jump_left = 0; }
			else if (place_meeting(x,y+global.grav,objBlockRefresher))
				{ jump_left = instance_place(x,y+global.grav,objBlockRefresher).num_jumps - 1; }
			else
				{ reset_jumps(); }
...
function reset_jumps() {
	with (objPlayer) {
		var _plat_disfresher, _plat_refresher;
		_plat_disfresher = instance_place(x,y+global.grav,objPlatformDisfresher);
		_plat_refresher = instance_place(x,y+global.grav,objPlatformRefresher);
		
		if (place_meeting(x,y+global.grav,objBlockDisfresher) || _plat_disfresher != noone)
		{
			jump_left = 0;
			if (_plat_disfresher != noone)
				{ with(_plat_disfresher) event_user(0); }
		}
		else if (place_meeting(x,y+global.grav,objBlockRefresher))
			{ jump_left = instance_place(x,y+global.grav,objBlockRefresher).num_jumps - 1; }
		else if (_plat_refresher != noone)
			{ jump_left = _plat_refresher.num_jumps - 1; }
		else
